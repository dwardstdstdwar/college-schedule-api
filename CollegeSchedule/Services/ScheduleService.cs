...
namespace CollegeSchedule.Services
{
    public class ScheduleService : IScheduleService
    {
        ...
        public async Task<List<ScheduleByDateDto>> GetScheduleForGroup(string groupName, DateTime startDate, DateTime endDate)
        {
            ...
            return BuildScheduleDto(startDate, endDate, schedules); // ← ИСПРАВЛЕНО
        }
        ...
        private static LessonDto BuildLessonDto(IEnumerable<Schedule> lessonGroup) // ← ИСПРАВЛЕНО
        {
            var first = lessonGroup.First();
            var lessonDto = new LessonDto
            {
                LessonNumber = first.LessonTime.LessonNumber,
                Time = $"{first.LessonTime.TimeStart:hh\\:mm}-{first.LessonTime.TimeEnd:hh\\:mm}",
